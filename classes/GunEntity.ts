...
import PaintballProjectileEntity from './PaintballProjectileEntity';
import type { ItemEntityOptions } from './ItemEntity';

// --- Projectile Speed Constant ---
const PROJECTILE_SPEED = 50; // Speed in meters per second for the projectile
// --- End Projectile Speed Constant ---
...
export default abstract class GunEntity extends ItemEntity {
  ...
  public shoot(): void {
    ...
    // --- Spawn Paintball Projectile --- 
    const projectile = new PaintballProjectileEntity({
      shooter: player,
      initialDamage: this.damage, // Pass the gun's base damage
    });
    projectile.spawn(player.world, origin); // Spawn at the calculated origin
    projectile.initiate({ 
      x: direction.x * PROJECTILE_SPEED,
      y: direction.y * PROJECTILE_SPEED,
      z: direction.z * PROJECTILE_SPEED
    });
    // --- End Spawn Paintball Projectile ---
  }
  ...
  protected getShootOriginDirection(): { origin: Vector3Like, direction: Vector3Like } {
    const player = this.parent as GamePlayerEntity;
    const playerPos = Vector3.fromVector3Like(player.position);
    const cameraOffset = player.player.camera.offset;
    const directionVec = Vector3.fromVector3Like(player.player.camera.facingDirection);

    // Start at the player's position + camera offset (eye level)
    const eyePosition = playerPos.add(Vector3.fromVector3Like(cameraOffset));

    // Move slightly forward from the eye position in the direction the camera is facing
    // Adjust the multiplier (e.g., 0.5) to position the spawn point appropriately relative to the gun model
    // Scale the direction vector components manually
    const offsetFactor = 0.5;
    const spawnOffset: Vector3Like = {
      x: directionVec.x * offsetFactor,
      y: directionVec.y * offsetFactor,
      z: directionVec.z * offsetFactor,
    };
    const originVec = eyePosition.add(Vector3.fromVector3Like(spawnOffset)); // Use the scaled offset

    return {
      origin: { x: originVec.x, y: originVec.y, z: originVec.z },
      direction: { x: directionVec.x, y: directionVec.y, z: directionVec.z } // Return the original direction
    };
  }
  ...
  /**
   * Updates the ammo indicator UI for the player.
   * Sends the current ammo count or reloading status.
   */
  public updateAmmoIndicatorUI(reloading: boolean = false): void {
    ...
    // Only send current ammo or reloading status
    player.player.ui.sendData(reloading ? {
      type: 'ammo-indicator',
      reloading: true,
      show: true, // Always show when reloading
    } : {
      type: 'ammo-indicator',
      ammo: this.ammo,
      show: true,

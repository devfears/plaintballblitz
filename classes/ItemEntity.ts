...
export default class ItemEntity extends Entity {
  ...
  public drop(fromPosition: Vector3Like, direction: Vector3Like): void {
    ...
    setTimeout(() => {
      // Check if the entity is still valid before applying impulse
      if (this.isSpawned && this.rawRigidBody) {
        this.applyImpulse({
          x: direction.x * this.mass * 7,
          y: direction.y * this.mass * 15, // Increased vertical impulse for better visual effect
          z: direction.z * this.mass * 7,
        });
      } else {
        // Optionally log if the impulse was skipped because the entity despawned
        // console.log(`Skipped applyImpulse for item ${this.name || 'unknown'} as it was already despawned.`);
      }
    }, 0);
